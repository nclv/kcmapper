...
class RoleProcessor:
    ...
    def process(self, clients, realm_name):
        ...
        for client in clients:
            try:
                client_roles = self.keycloak_admin.get_client_roles(
                    client_id=client["id"], brief_representation=False
                )
                all_client_roles.extend(client_roles)
            except Exception as e:
                self.logger.error(
                    "  Error extracting roles for client %s: %s", client["clientId"], e
                )
        self.logger.info(
            "  -> Found %s client role(s) across all clients.", len(all_client_roles)
        )
        ...
        for role in roles:
            ...
            try:
                ...
                if role.get("clientRole"):
                    child_roles = self.keycloak_admin.get_role_client_level_children(
                        client_id=role["containerId"], role_id=role["id"]
                    )
                else:
                    child_roles = self.keycloak_admin.get_composite_realm_roles_of_role(
                        role_name=role["name"]
                    )
                ...
                for child_role in child_roles:
                    ...
                    execute_query(
                        ...
                        self.logger,
                        query,
                        {"parent_id": role["id"], "child_id": child_role["id"]},
                    )
            except Exception as e:
                self.logger.error(
                    "Error processing composite roles for role %s (%s): %s",
                    role["name"],
                    role["id"],
                    e,
                )
