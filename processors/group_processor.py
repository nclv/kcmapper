...
class GroupProcessor:
    ...
    def _load_group_recursive(self, group, realm_name, parent_id, clients):
        ...
        properties = clean_properties(
            group, keys_to_ignore={"id", "subGroups", "roleMappings"}
        )
        ...
        if parent_id:
            ...
            execute_query(
                ...
                self.logger,
                query_link_parent,
                {"parent_id": parent_id, "child_id": group_id},
            )
        else:
        ...
        if "subGroups" in group and group["subGroups"]:
            for subgroup in group["subGroups"]:
                self._load_group_recursive(
                    subgroup, realm_name, parent_id=group_id, clients=clients
                )
    ...
    def _process_group_role_mappings(self, group_id, realm_name, clients):
        ...
        try:
            ...
            self.logger.error(
                "  Error extracting realm roles for group %s: %s", group_id, e
            )
        ...
        for client in clients:
            ...
            try:
                client_roles = self.keycloak_admin.get_group_client_roles(
                    group_id=group_id, client_id=client_internal_id
                )
                ...
                self.logger.error(
                    "  Error extracting client roles for group %s and client %s: %s",
                    group_id,
                    client_internal_id,
                    e,
                )
