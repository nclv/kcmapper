...
class UserProcessor:
    ...
    def _process_user_role_mappings(self, user_id, realm_name, clients):
        try:
            direct_realm_roles = self.keycloak_admin.get_realm_roles_of_user(
                user_id=user_id
            )
            ...
            self.logger.error(
                "  Error processing realm roles for user %s: %s", user_id, e
            )

        for client in clients:
            client_internal_id = client["id"]
            try:
                direct_client_roles = self.keycloak_admin.get_client_roles_of_user(
                    user_id=user_id, client_id=client_internal_id
                )
                ...
                self.logger.error(
                    "  Error processing client roles for user %s and client %s: %s",
                    user_id,
                    client_internal_id,
                    e,
                )
