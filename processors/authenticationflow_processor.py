...
class AuthenticationFlowProcessor:
    ...
    def _extract(self):
        ...
        try:
            ...
            self.logger.error(
                "  Error while extracting auth flows: %s", e, exc_info=True
            )
            return []
    ...
    def _load(self, flows, realm_name):
        ...
        for flow in flows:
            properties = clean_properties(
                flow, keys_to_ignore={"id", "alias", "authenticationExecutions"}
            )
    ...
    def _load_execution(self, execution, flow_id):
        ...
        if not execution_id:
            self.logger.debug(
                "Skipping execution without an ID in flow %s: %s", flow_id, execution
            )
            return
