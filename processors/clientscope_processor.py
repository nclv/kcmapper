...
class ClientScopeProcessor:
    ...
    def _extract(self):
        ...
        try:
            ...
            self.logger.error(
                "  Error while extracting client scopes: %s", e, exc_info=True
            )
            return []
    ...
    def _process_client_scope_mappings(self, clients):
        ...
        for client in clients:
            try:
                default_scopes = self.keycloak_admin.get_client_default_client_scopes(
                    client["id"]
                )
                for scope in default_scopes:
                    ...
                    execute_query(
                        ...
                        self.logger,
                        query,
                        {"client_id": client["id"], "scope_id": scope["id"]},
                    )
            except Exception as e:
                self.logger.error(
                    "  Error getting default scopes for client %s: %s",
                    client["clientId"],
                    e,
                )
            try:
                optional_scopes = self.keycloak_admin.get_client_optional_client_scopes(
                    client["id"]
                )
                ...
                self.logger.error(
                    "  Error getting optional scopes for client %s: %s",
                    client["clientId"],
                    e,
                )
    ...
    def _process_mappers(self, scope_id):
        try:
            ...
            self.logger.error(
                "  Error getting mappers for client scope %s: %s", scope_id, e
            )
