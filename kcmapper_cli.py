...
from utils.neo4j_utils import clean_database, initialize
...
def run_export(args, logger):
    ...
    try:
        ...
        initialize(neo4j_driver, args.neo4j_database, logger)

        if args.clean:
            clean_database(neo4j_driver, args.neo4j_database, logger)
        ...
        logger.info(
            "Successfully connected to Keycloak (realm '%s').", args.keycloak_realm
        )

        # Instantiate all processors
        realm_processor = RealmProcessor(
            keycloak_admin, neo4j_driver, args.neo4j_database, logger
        )
        client_processor = ClientProcessor(
            keycloak_admin, neo4j_driver, args.neo4j_database, logger
        )
        role_processor = RoleProcessor(
            keycloak_admin, neo4j_driver, args.neo4j_database, logger
        )
        ...
        clientscope_processor = ClientScopeProcessor(
            keycloak_admin, neo4j_driver, args.neo4j_database, logger
        )
        authflow_processor = AuthenticationFlowProcessor(
            keycloak_admin, neo4j_driver, args.neo4j_database, logger
        )
        idp_processor = IdpProcessor(
            keycloak_admin, neo4j_driver, args.neo4j_database, logger
        )
...
def run_analyzer(args):
    ...
    try:
        ...
        web_app.config["NEO4J_DATABASE"] = args.neo4j_database
        logger.info(
            f"--> KcMapper Analyzer available at http://{args.host}:{args.port}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="KcMapper: A tool to export and analyze Keycloak data.",
    ...
    parser_export = subparsers.add_parser(
        "export", help="Export data from Keycloak to Neo4j."
    )
    ...
    log_group.add_argument(
        "--log-file", default="kcmapper_export.log", help="Path to the log file."
    )
    kc_group = parser_export.add_argument_group("Keycloak Connection")
    kc_group.add_argument(
        "--keycloak-url",
        default=os.getenv("KEYCLOAK_URL"),
        required=not os.getenv("KEYCLOAK_URL"),
        help="Keycloak server URL",
    )
    kc_group.add_argument(
        "--keycloak-realm",
        default=os.getenv("KEYCLOAK_REALM", "master"),
        help="Keycloak realm for admin auth",
    )
    kc_group.add_argument(
        "--keycloak-client-id",
        default=os.getenv("KEYCLOAK_CLIENT_ID", "admin-cli"),
        help="Client ID for the admin API",
    )
    kc_group.add_argument(
        "--keycloak-user",
        default=os.getenv("KEYCLOAK_USER"),
        required=not os.getenv("KEYCLOAK_USER"),
        help="Keycloak admin username",
    )
    kc_group.add_argument(
        "--keycloak-password",
        default=os.getenv("KEYCLOAK_PASSWORD"),
        help="Keycloak admin password (or use env var)",
    )
    kc_group.add_argument(
        "--keycloak-secret",
        default=os.getenv("KEYCLOAK_CLIENT_SECRET"),
        help="Keycloak client secret (for confidential clients)",
    )
    kc_group.add_argument(
        "--keycloak-totp",
        default=os.getenv("KEYCLOAK_TOTP"),
        help="Time-based One-Time Password for 2FA",
    )
    kc_group.add_argument(
        "--keycloak-cert-path",
        default=os.getenv("KEYCLOAK_CERT_PATH"),
        help="Path to client certificate for mTLS",
    )

    neo_group_export = parser_export.add_argument_group("Neo4j Connection")
    neo_group_export.add_argument(
        "--neo4j-uri",
        default=os.getenv("NEO4J_URI", "bolt://localhost:7687"),
        help="Neo4j connection URI",
    )
    neo_group_export.add_argument(
        "--neo4j-user", default=os.getenv("NEO4J_USER", "neo4j"), help="Neo4j username"
    )
    neo_group_export.add_argument(
        "--neo4j-password",
        default=os.getenv("NEO4J_PASSWORD"),
        help="Neo4j password (or use env var)",
    )
    neo_group_export.add_argument(
        "--neo4j-database",
        default=os.getenv("NEO4J_DATABASE", "system"),
        help="Neo4j database",
    )
    parser_export.add_argument(
        "--clean", action="store_true", help="Clean Neo4j DB before extraction."
    )

    # --- ANALYZE Sub-command ---
    parser_analyze = subparsers.add_parser(
        "analyze", help="Launch the web UI for interactive analysis."
    )
    neo_group_analyze = parser_analyze.add_argument_group(
        "Neo4j Connection (for Web UI)"
    )
    neo_group_analyze.add_argument(
        "--neo4j-uri",
        default=os.getenv("NEO4J_URI", "bolt://localhost:7687"),
        help="Neo4j connection URI",
    )
    ...
    web_group.add_argument(
        "--port", type=int, default=5001, help="Port for the web server."
    )
    ...
    if args.command == "export":
        if not args.keycloak_password:
            sys.exit("Error: Keycloak password is required.")
        if not args.neo4j_password:
            sys.exit("Error: Neo4j password is required.")
        logger = setup_logging(args.log_level, args.log_file)
        run_export(args, logger)
    elif args.command == "analyze":
        if not args.neo4j_password:
            sys.exit("Error: Neo4j password is required for the web UI.")
        run_analyzer(args)
