...
def get_autocomplete_data():
    ...
    try:
        ...
        database = web_app.config["NEO4J_DATABASE"]
        data = request.json
        ...
        if not query_details:
            return (
                jsonify({"error": f"Source query '{source_query_name}' not found."}),
                404,
            )
        ...
        with driver.session(database=database) as session:
            ...
            suggestions = [
                record.values()[0] for record in result if record.values()[0]
            ]
...
def execute_api_query():
    ...
    try:
        ...
        return (
            jsonify({"error": "Neo4j authentication failed. Check credentials."}),
            401,
        )
